...
class PyAvataaar:
    ...
    def __render_svg(self) -> str:
        env = Environment(
            loader=PackageLoader('PhythonAvatarGenerator', 'templates'),
            trim_blocks=True,
            lstrip_blocks=True,
            autoescape=True,
...
class AvatarInfo(BaseModel):
    ...
    def make_id(self) -> str:
        """
        Generate a short, descriptive ID based on the current avatar information.
        """
        avatar_id_parts = [
            self.style.value if isinstance(self.style, enum.Enum) else self.style,
            self.skin_color.value if isinstance(self.skin_color, enum.Enum) else self.skin_color,
            self.hair_color.value if isinstance(self.hair_color, enum.Enum) else self.hair_color,
            self.top_type.value if isinstance(self.top_type, enum.Enum) else self.top_type,
            self.facial_hair_type.value if isinstance(self.facial_hair_type, enum.Enum) else self.facial_hair_type,
            self.mouth_type.value if isinstance(self.mouth_type, enum.Enum) else self.mouth_type,
            self.eye_type.value if isinstance(self.eye_type, enum.Enum) else self.eye_type,
            self.eyebrow_type.value if isinstance(self.eyebrow_type, enum.Enum) else self.eyebrow_type,
            self.accessories_type.value if isinstance(self.accessories_type, enum.Enum) else self.accessories_type,
            self.clothe_type.value if isinstance(self.clothe_type, enum.Enum) else self.clothe_type,
            self.clothe_color.value if isinstance(self.clothe_color, enum.Enum) else self.clothe_color,
            self.clothe_graphic_type.value if isinstance(self.clothe_graphic_type, enum.Enum) else self.clothe_graphic_type,
        ]
        avatar_id = ''.join(f'{x:02x}' for x in avatar_id_parts)
        return avatar_id

    @classmethod
    def from_id(cls, avatar_id: str):
        """
        Create an AvatarInfo instance from a given ID.
        """
        if len(avatar_id) != 24:
            raise ValueError("Invalid avatar ID length")

        try:
            id_parts = [int(avatar_id[i:i+2], 16) for i in range(0, len(avatar_id), 2)]
            return cls(
                style=AvatarStyle(id_parts[0]),
                skin_color=SkinColor(id_parts[1]),
                hair_color=HairColor(id_parts[2]),
                top_type=TopType(id_parts[3]),
                facial_hair_type=FacialHairType(id_parts[4]),
                mouth_type=MouthType(id_parts[5]),
                eye_type=EyesType(id_parts[6]),
                eyebrow_type=EyebrowType(id_parts[7]),
                accessories_type=AccessoriesType(id_parts[8]),
                clothe_type=ClotheType(id_parts[9]),
                clothe_color=Color(id_parts[10]),
                clothe_graphic_type=ClotheGraphicType(id_parts[11]),
            )
...
class AvatarMaker:
    ...
    @staticmethod
    def create_avatar_from_info_id(avatar_info_id: str, output_type: str = 'svg') -> Union[str, bytes]:
        try:
            avatar = PyAvataaar(**AvatarInfo.from_id(avatar_info_id).dict())
            if output_type.lower() == 'svg':
                return avatar.render_svg()
            elif output_type.lower() == 'png':
                return avatar.render_png()
